...
bool loadTexture(char * fileName, int & handle)
{
	...
	string file = "Assets\\" + (string)fileName;
	bool success = readBMP(file.c_str(), data, w, h);
	if(!success)
	{
...
bool getObjData(const char* fileName, vector<material> & materials,
		vector<vertexData> & vertexBuffer, bool & hasUV, bool & hasNormals)
{
	...
	{
		// Read in a file
		string mtlf = "Assets\\" + (string)mtlFiles[i].c_str();
		FILE* fp;
		fp = fopen(mtlf.c_str(), "r");
		if(fp == NULL)
		{
